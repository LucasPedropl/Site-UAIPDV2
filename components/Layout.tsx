...
const PartnerLogo = ({ name, src, color }: { name: string, src: string, color: string }) => {
    const [error, setError] = useState(false);

    if (error) {
        return (
            <span className={`text-lg font-bold ${color} px-2 opacity-70 hover:opacity-100 transition-opacity`}>{name}</span>
        );
    }
