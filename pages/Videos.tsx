...
export const Videos = ({ content }: { content: any }) => {
	...
	return (
		<>
				...
				<div className="relative container mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
					<div className="space-y-8">
						...
						<div className="flex flex-wrap gap-4">
							<a
								href="#video-gallery"
								className="inline-flex items-center justify-center px-8 py-3.5 rounded-full bg-brand-blue text-sm font-bold shadow-lg shadow-brand-blue/30 hover:bg-brand-blue-dark transition-all"
							>
								Assistir agora
							</a>
						</div>
						...
						<div className="relative rounded-[32px] border border-white/10 bg-white/5 p-4 backdrop-blur-md">
							<div className="rounded-3xl overflow-hidden bg-gray-900/50 aspect-[4/3]">
								<img
									...
									onError={(e) => {
										e.currentTarget.src =
											'https://placehold.co/800x600?text=Platform+Demo';
										e.currentTarget.onerror = null;
									}}
								/>
							</div>
				...
				<div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
					<div className="max-w-3xl mb-16">
						...
						<p className="mt-3 text-white/70 max-w-2xl text-lg">
							Visualize rapidamente onde cada módulo atua e como
							ele conecta a operação do PDV à retaguarda.
						</p>
					</div>
								...
								<div className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-8 flex gap-6 hover:bg-white/10 transition-colors">
									<div className="flex-shrink-0 flex h-16 w-16 items-center justify-center rounded-2xl bg-white/10">
										<img
											...
											onError={(e) => {
												e.currentTarget.src =
													'https://placehold.co/60x60?text=Mod';
												e.currentTarget.onerror = null;
											}}
										/>
									</div>
			...
			<section
				id="video-gallery"
				className="relative py-24 text-white overflow-hidden bg-[#010410]"
			>
				{/* Background Glows */}
				...
				<div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
					<div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-12">
						...
						<p className="text-white/80 max-w-2xl text-lg">
							Escolha um módulo para assistir e avance pela
							jornada do operador: abertura de caixa, lançamento,
							entrega e gestão completa em nuvem.
						</p>
					</div>
						...
						<div className="flex items-center justify-center gap-2 sm:gap-3 p-2 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm w-fit mx-auto min-w-min">
							{content.categories.map((category: string) => (
								<button
									...
									className={`px-3 sm:px-6 py-2 sm:py-3 rounded-xl font-semibold text-xs sm:text-sm transition-all whitespace-nowrap ${
								...
								<article className="group rounded-3xl bg-[#0a0f1e] border border-white/5 p-6 shadow-xl hover:border-brand-blue/30 transition-all hover:-translate-y-1">
									<div className="aspect-video rounded-2xl overflow-hidden bg-black/40 ring-1 ring-white/10 mb-6 relative">
										<iframe
											...
											allowFullScreen
										/>
										<span className="absolute top-3 right-3 px-3 py-1 rounded-full bg-brand-blue/90 backdrop-blur-sm text-xs font-bold text-white shadow-lg">
											{video.category}
										</span>
									</div>
