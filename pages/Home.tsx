...
const StatCounter: React.FC<{ number: string; label: string }> = ({
	number,
	label,
}) => {
	const { ref, isVisible } = useFadeIn();
				...
				<div className="absolute inset-0 z-0">
					{heroSlides.map((slide: any, index: number) => (
						<img
							...
							onError={(e) => {
								e.currentTarget.src =
									'https://placehold.co/1920x1080?text=Slide+Hero';
								e.currentTarget.onerror = null;
							}}
						/>
					))}
					{/* Overlay cleaner and more neutral */}
			...
			<section className="py-16 sm:py-24 bg-white">
				<div className="container mx-auto px-4 sm:px-6 lg:px-8">
					<div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
						<div>
							<p className="text-sm uppercase tracking-widest text-brand-blue mb-2 font-semibold">
								Demonstrações
							</p>
					...
					<div className="relative mt-12 group">
						...
						>
							{content.videos.map((video: any) => (
								<div
									key={video.id}
									className="bg-white border border-gray-200 rounded-2xl p-6 w-80 md:w-96 flex-shrink-0 shadow-sm snap-center hover:shadow-md transition-shadow"
								>
								...
								<article className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 h-full hover:shadow-md transition-shadow">
									<div className="h-48 rounded-2xl overflow-hidden mb-6 bg-gray-100 flex items-center justify-center p-4">
										<img
											...
											onError={(e) => {
												e.currentTarget.src =
													'https://placehold.co/400x300?text=Equipamento';
												e.currentTarget.onerror = null;
											}}
										/>
									</div>
								...
								<article className="bg-white border border-gray-200 rounded-2xl p-5 flex flex-col items-center text-center h-full hover:shadow-lg hover:border-brand-blue/30 transition-all cursor-pointer group">
									<div className="w-24 h-24 rounded-full overflow-hidden mb-4 ring-2 ring-gray-100 group-hover:ring-brand-blue/20 transition-all">
										<img
											...
											onError={(e) => {
												e.currentTarget.src =
													'https://placehold.co/200x200?text=Seg';
												e.currentTarget.onerror = null;
											}}
										/>
									</div>
									<h3 className="mt-4 text-lg font-semibold text-gray-900 group-hover:text-brand-blue transition-colors">
										{seg.title}
									</h3>
									<p className="mt-2 text-gray-600 text-sm">
										{seg.desc}
				...
				<div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
					...
					<div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
						{content.benefits.map((ben: any, idx: number) => (
							<article
								key={idx}
								className="bg-white/5 rounded-2xl p-6 border border-white/10 hover:bg-white/10 transition-colors backdrop-blur-sm"
							>
					...
					<div>
						...
						<ul className="mt-8 space-y-6">
							{content.modules.map((mod: any, idx: number) => (
								<li
									key={idx}
									className="p-6 border border-gray-200 rounded-2xl hover:border-brand-blue/30 hover:shadow-md transition-all bg-white"
								>
									<h3 className="text-xl font-semibold text-gray-900 flex items-center gap-2">
										<span className="w-2 h-2 rounded-full bg-brand-blue inline-block"></span>
										{mod.title}
									</h3>
									<p className="mt-2 text-gray-600 pl-4 border-l-2 border-gray-100">
										{mod.desc}
									</p>
								</li>
							))}
					...
					<div className="grid lg:grid-cols-2 gap-12 items-start">
						<div>
							...
							>
								...
								<button
									type="submit"
									className="inline-flex items-center justify-center rounded-full px-8 py-3.5 bg-brand-blue text-white font-semibold hover:bg-brand-blue-dark transition-all shadow-lg shadow-brand-blue/20 transform hover:-translate-y-0.5"
								>
									Enviar Mensagem
								</button>
							</form>
						...
						<div className="bg-white rounded-3xl shadow-xl p-8 lg:p-10 border border-gray-100">
							...
							<p className="text-gray-600 leading-relaxed">
								Entre em contato e descubra como acelerar o seu
								ponto de venda com a UAI PDV.
							</p>
